...
@endif
    ...
    // 🔥 EDIT MODE AUTO-FILL
    if (window.isEdit && window.employeeData) {
        fillFormForUpdate(window.employeeData);
    }
};
        ...
        document.getElementById("onboardingForm").addEventListener("submit", function(e) {
            ...
            // ✅ AJAX to Laravel
            fetch("/employee-onboarding/store", {
                    method: "POST",
                    headers: {
    ...
    <script>
function fillFormForUpdate(data) {
    if (!data) return;

    // BASIC INFO
    employeeType.value = data.employee_type ?? '';
    ...
    contractEndDate.value = data.contract_end_date ?? '';
    probationPeriod.value = data.probation_period_months ?? 6;
    updateProbationEndDate();

    // EMERGENCY
    emergencyName.value  = data.emergency_name ?? '';
    emergencyPhone.value = data.emergency_phone ?? '';
    emergencyRelation.value = data.emergency_relation ?? '';
    ...
    workArea.value      = data.work_area ?? '';
    shift.value         = data.shift ?? '';
    reportingManager.value = data.reporting_manager ?? '';

    // HRMS
    hrmsAccess.value   = data.hrms_access_level ?? '';
    accessExpiry.value = data.access_expiry ?? '';
    ...
    paymentDate.value  = data.payment_date_day ?? '';

    bankName.value      = data.bank_name ?? '';
    accountNumber.value = data.account_number ?? '';
    ifscCode.value      = data.ifsc_code ?? '';
    uanNumber.value     = data.uan_number ?? '';

    additionalNotes.value = data.additional_notes ?? '';

    // HRMS MODULES
    // HRMS MODULES (SAFE FOR ALL CASES)
if (data.hrms_modules) {
    let modules = [];

    if (Array.isArray(data.hrms_modules)) {
        // already array
        modules = data.hrms_modules;
    } else if (typeof data.hrms_modules === 'string') {
        // comma separated string
        modules = data.hrms_modules.split(',');
    } else if (typeof data.hrms_modules === 'object') {
        // JSON string stored as object
        try {
            modules = Object.values(data.hrms_modules);
        } catch (e) {
            modules = [];
        }
    }
    ...
    // DOCUMENT CHECKLIST
    // DOCUMENT CHECKLIST (SAFE FOR ALL CASES)
if (data.documents) {
    let docs = [];
    ...
    if (Array.isArray(data.documents)) {
        ...
        docs = data.documents.split(',');
    } else if (typeof data.documents === 'object') {
        try {
            docs = Object.values(data.documents);
        } catch (e) {
            docs = [];
        }
    }
    ...
    docs.forEach(d => {
        const cb = document.querySelector(`input[value="${d.trim()}"]`);
        if (cb) cb.checked = true;
    });
}
